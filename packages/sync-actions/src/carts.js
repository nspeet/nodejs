...
function createOrderMapActions(
  ...
  return function doMapActions(
    ...
    //sync cart lineitems
    allActions.push(
      mapActionGroup('lineItems', (): Array<UpdateAction> =>
        orderActions.actionsMapBase(diff, oldObj, newObj, syncActionConfig)
      )
    )

    //sync cart custom lineitems
    allActions.push(
      mapActionGroup('CustomLineItem', (): Array<UpdateAction> =>
        orderActions.actionsMapBase(diff, oldObj, newObj, syncActionConfig)
      )
    )
    ...
    //sync cart tax mode
    allActions.push(
      mapActionGroup('taxMode', (): Array<UpdateAction> =>
        orderActions.actionsMapBase(diff, oldObj, newObj, syncActionConfig)
      )
    )
    //consider shipping method, shipping address, discount codes, customer group,customer id, customer email, customer type
    allActions.push(
